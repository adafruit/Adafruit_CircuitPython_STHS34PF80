...
class STHS34PF80:
    ...
    def ambient_averaging(self, value: int) -> None:
        ...
        if value not in _AVG_T_VALUES:
            raise ValueError(f"Invalid averaging value. Must be one of: {list(_AVG_T_VALUES.keys())}")
        self._avg_t = value
    ...
    def object_averaging(self, value: int) -> None:
        ...
        if value not in _AVG_TMOS_VALUES:
            raise ValueError(f"Invalid averaging value. Must be one of: {list(_AVG_TMOS_VALUES.keys())}")
    ...
    def data_rate(self, value: int) -> None:
        ...
        if value > max_odr:
            raise ValueError(f"ODR {_ODR_VALUES[value]} exceeds maximum {_ODR_VALUES[max_odr]} for current averaging")
    ...
    def interrupt_signal(self, value: int) -> None:
        ...
        if value not in _INT_SIGNAL_VALUES:
            raise ValueError(f"Invalid interrupt signal value. Must be one of: {list(_INT_SIGNAL_VALUES.keys())}")
